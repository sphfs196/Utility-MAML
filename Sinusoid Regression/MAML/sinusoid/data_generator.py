...
class DataGenerator(object):
    ...
    def generate_sinusoid_batch(self, train=True, val=False, input_idx=None):
        ...
        # num_samples = 5 + 200
        phase = np.random.uniform(self.phase_range[0], self.phase_range[1], [num_batch])
        amp = np.zeros(num_batch)
        d = 0.01
        d_idx = (5-0.1)/num_batch
        if train and not val:
        ...
        for idx, func in enumerate(funcs): 
            # sample x points
            init_inputs[idx] = np.random.uniform(self.input_range[0], self.input_range[1], [num_samples, 1])
            if input_idx is not None:
                init_inputs[:,input_idx:,0] = np.linspace(self.input_range[0], self.input_range[1], num=num_samples-input_idx, retstep=False)
            if train:
                if func < 95 or val:
                    # d = 0.01
                    amp[idx] = np.random.uniform(0.1+func*d, 0.1+(func+1)*d, 1)
                else:
                    amp[idx] = np.random.uniform(4.95+(func-95)*d, 4.95+(func-94)*d, 1)
            else:
                amp[idx] = np.random.uniform(0.1, 5, 1)   
