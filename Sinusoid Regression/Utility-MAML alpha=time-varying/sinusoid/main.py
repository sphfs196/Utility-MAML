...
def trial(model, saver, sess, exp_string, data_generator, TR_MAML=False, resume_itr=0):
    ...
    train_mean_loss2 = np.zeros(250) # loss2 = avg or log expsum of outer loop loss(loosb)
    ...
    for itr in range_iters:
        ...
        if itr <= 30000.0 : 
            ...
            alpha = (9.0 / 20000.0) * float(itr) - 12.5
...
#             input_tensors.extend([model.summ_op, model.total_loss1, model.total_losses2[FLAGS.num_updates-1]])
        ...
        if itr % PRINT_INTERVAL == 0:
            ...
            train_mean_loss2[iter_ind] = result[-1]

            print('loss to sum in logexp= ', result[-3])
          
            # for training
            print_str += ': ' + str(train_max[iter_ind]) + ', ' + str(train_mean[iter_ind]) + ', ' + str(np.argmax(postloss_by_idx)) + ', ' + str(train_mean_loss2[iter_ind])
            print(print_str)
            postlosses = []
        ...
        if itr % VAL_INTERVAL == 0 :
            for jj in range(NUM_TEST_POINTS):
                ...
                feed_dict[model.alpha] = float(alpha)
                result = sess.run(input_tensors, feed_dict)
    ...
    if FLAGS.train:
        ...
        np.savetxt("logs_stats/train_mean_loss2" +str(FLAGS.log_number) + ".csv", train_mean_loss2, delimiter=",")
        ...
        print('val_losses = ')
        print(val_losses)
        print('val_idx_counts = ')
        print(val_idx_counts)
        print('sum(val_idx_counts) = ')
        print(sum(val_idx_counts))
        val_losses = np.divide(val_losses, val_idx_counts)
